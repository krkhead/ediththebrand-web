...
import { ADMIN_EMAIL, clerkRuntime } from "@/lib/clerk-config";
...
export default async function AdminLayout({
  ...
  if (adminAuth.enabled && !adminSession) {
        ...
        <aside className="w-64 bg-[#3D2E24] flex flex-col fixed h-full z-10">
          ...
          <div className="p-4 border-t border-[#5A4535] space-y-3">
            ...
            <div className="px-4 space-y-2">
              <p className="text-xs text-[#8A7D72]">{adminSession.email}</p>
              <form action="/admin/sign-out" method="post">
                <button className="w-full border border-[#E8A020] px-3 py-2 text-xs tracking-[0.2em] uppercase text-[#E8A020] transition-colors hover:bg-[#E8A020] hover:text-[#3D2E24]">
                  Sign Out
                </button>
              </form>
            </div>
